			...
			loadData(feature, params, startingState, sessionData, store, Locale).then((action) => {
				...
				function finish() {
					...
					getRenderProps(feature, params.url).then((renderProps) => {
						...
						res.render('standalone', { appString: html, initialState, environment: process.env.NODE_ENV, getAssetPath, assetPrefix, config: getConfig(feature), locale: Locale, nodeHost: getNodeHost(req), railsHost: params.railsHost, referrer: params.referrer }, (err, html) => {
