  ...
  class Application < Rails::Application
    config.middleware.insert_before(Rack::Lock, Rack::Rewrite) do
      ...
      mobile_rewrite = lambda do |path, rack_env|
        ...
        # If this is a mobile client asking for the /mobile page
        # - forward them to "http://m.youversion.com/download"
        # - where there is a script to detect proper app store redirection.
      ...
      should_rewrite_mobile = Proc.new do |rack_env|
        ...
        mobile = !rack_env["X_MOBILE_DEVICE"].nil? && rack_env["PATH_INFO"] !~ /(\/app$|\/settings\/notifications|\/users\/\w+(-|_)*(?:-|_|\w+)*\/notifications|^(\/.{2,5})?\/terms|privacy|^\/status$|^\/assets\/|^\/videos(\/)*[0-9]*(\/)*\w*)/
