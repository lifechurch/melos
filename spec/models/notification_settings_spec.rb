...
describe NotificationSettings do
  ...
  describe "#update" do
    it "sets a user's notification settings" do
      ...
      ns = NotificationSettings.find(auth: auth)
      ns.update({badges: "0", follower: "0", newsletter: "0", note_like: "0", reading_plans: "0", partners: "1"})
      ns = NotificationSettings.find(auth: auth)
      ns.badges.should be_false
      ns.update({badges: "1", follower: "1", newsletter: "1", note_like: "1", reading_plans: "1", partners: "1"})
      ns = NotificationSettings.find(auth: auth)
      ns.user.should == @paranoid_user
      ns.badges.should == true
    end
  end

  describe "create path" do
    it "should be tested to ensure coverage" do
      NotificationSettings.create_path.should == "users/update_notification_settings"
    end
  end
