  ...
  module Moments
    class Feed
      ...
      def client_side_moments
        ...
        @client_side_moments = @client_side_moments.select{|m| m.expanded_dt.any?{|d| d.to_date <= Date.today } unless m.is_a?(GenericMoment) } if @client_side_moments.present?
        ...
        @client_side_moments.each { |m| m.created_dt = m.expanded_dt.select{|d| d.to_date <= Date.today }.max.to_s unless m.is_a?(GenericMoment) } if @client_side_moments.present?
      end
