...
function getKeysAndValue(key) {
	...
	if (keys.length < 1) {
		throw `LocalStore: Invalid key for setIn/getIn: ${key}. Must contain at least two keys delimited by a period.`
	}
	...
	if (typeof storageObject === 'undefined') {
		...
		throw `LocalStore: An non-object value already exists with the key ${storageKey}`
	}
