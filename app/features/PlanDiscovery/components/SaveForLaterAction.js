...
class SaveForLaterAction extends Component {
	...
	render() {
		...
		if (readingPlan.saved) {
			if (button) {
				var saveforlater = <a className='save-for-later button' onClick={this.saveForLater.bind(this, readingPlan.id)}><CheckMark /><FormattedMessage id="plans.save for later" /> </a>
			} else {
