...
class PreviewSidebar extends Component {
	...
	publishEvent() {
		...
		dispatch(ActionCreators.publishEvent({
			...
			if (typeof errors === 'undefined') {
				dispatch(push(`/${params.locale}/event/edit/${event.item.id}/share`))
			}
		}, (error) => {
	...
	unpublishEvent() {
		...
		dispatch(ActionCreators.unpublishEvent({
			...
			const { errors } = response
			if (typeof errors === 'undefined') {
				dispatch(push(`/${params.locale}/event/edit/${event.item.id}/content`))
			}
		}, (error) => {
	...
	render() {
		...
		let times = locations ? Object.keys(locations).map((i) => { return locations[i].times }) : []
		times = times.concat.apply([], times) // flatten times
		let earliest = times.map((i) => { return new Date(i.start_dt) })
		let latest = times.map((i) => { return new Date(i.end_dt) })
		...
		return (
			<div className='sidebar'>
				...
				<div className='section'>
					...
					<h3 className="right">{earliest ? yvFormat(moment(earliest).subtract(5, 'days')) : null}</h3>
				...
				<div className='section'>
					...
					<h3 className="right">{earliest ? yvFormat(moment(earliest).subtract(30, 'minutes')) : null}</h3>
				...
				<div className='section'>
					...
					<h3 className="right">{latest ? yvFormat(moment(latest)) : null}</h3>
